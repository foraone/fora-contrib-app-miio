...
for docker_arch in ${TARGET_ARCHES}; do
  ...
  sed --in-place='' "s|__BASEIMAGE_ARCH__|${docker_arch}|g" Dockerfile.${docker_arch}
  sed --in-place='' "s|__QEMU_ARCH__|${qemu_arch}|g" Dockerfile.${docker_arch}
  if [[ ${docker_arch} == "amd64" || ${build_os} == "darwin" ]]; then
    sed --in-place='' "/__CROSS_/d" Dockerfile.${docker_arch}
  else
    sed --in-place='' "s/__CROSS_//g" Dockerfile.${docker_arch}
  fi
